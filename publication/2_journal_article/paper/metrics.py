...
Inputs:  the site-day decision-and-impact table of all methods (reference.py).
Outputs: pooled table (method × evaluation group), per-station table, macro table (the
         ...
         bootstrap intervals, the M9 confidence-versus-coverage table, the gate
         decision, the Beta 'unsure' sensitivity table and M9 calibration reliability.
Key steps: the four headline metrics are Reference Energy IoU, reference energy
         precision, site-day F1 and site-day precision (``headline``: energies pooled
         by summing before dividing, ``pooled_energy`` and ``day_metrics``); supporting
...
from pynrpf.m9 import AUTO_CORRECT, AUTO_KEEP, OUTCOMES, UNCERTAIN
...
SURE_CONFIDENCE = ("sure", "controlled")   # the reviewers' 'sure' label; every Alpha day is controlled truth


# ----------------------------------------------------------------------------- the headline definitions

def pooled_energy(t: pd.DataFrame, applied: np.ndarray) -> tuple[float, float]:
    """Pooled (Energy IoU, energy precision) over site-days; ``applied`` marks the days corrected.

    Energies are summed over the group before dividing, so a large day counts for
    more than a small one. Energy IoU is 0 when nothing is proposed and nothing is
    required; energy precision is undefined (NaN) when nothing is proposed.
    """
...
def headline(t: pd.DataFrame) -> dict:
    """The headline numbers for one group of site-days (a cohort or one station)."""
    ...
    # Human-parity number: the share of 'sure' RPF days that were corrected.
    sure = t[(t["rpf"] == 1) & (t["confidence"].isin(SURE_CONFIDENCE))]
...
def calibration_reliability_terms(p: np.ndarray, label: np.ndarray, bins: int = 10) -> dict:
...
def methods_present(site_days: pd.DataFrame) -> list[str]:
    """The methods in a site-day table, in the canonical order."""
...
def supporting(t: pd.DataFrame) -> dict:
    ...
    return dict(
        ...
        min_change_days=len(changed),
        min_change_mean_mw=float(changed["min_change_mw"].mean()) if len(changed) else np.nan,
        min_change_total_mw=float(changed["min_change_mw"].sum()),
        reference_min_change_days=len(ref_changed),
        reference_min_change_mean_mw=(float(ref_changed["reference_min_change_mw"].mean()) if len(ref_changed)
                                      else np.nan),
    )


def summarise(t: pd.DataFrame) -> dict:
    """Headline plus supporting metrics for one group of site-days."""
    return {**headline(t), **supporting(t)}
...
def macro_table(site_days: pd.DataFrame) -> pd.DataFrame:
    ...
    a station without RPF days is 0 by definition and stays in the mean.
    ...
    for method in methods_present(site_days):
        ...
        for group in GROUPS:
            ...
            for _ in range(draws):
                pick = [by_station[(c, s)] for c, stations in use.items()
                        for s in rng.choice(stations, size=len(stations), replace=True)]
                m = headline(pd.concat(pick, ignore_index=True))
    ...
    for cohort in ("alpha", "beta"):
        ...
        for c in settings["metrics"]["coverage_grid"]:
            ...
            rows.append(dict(
                ...
                auto_days=len(auto), auto_precision=m["day_precision"],
                auto_recall=float((correct & rpf).sum() / rpf.sum()) if rpf.sum() else np.nan,
...
def gate_decision(pooled: pd.DataFrame, settings: Settings) -> dict:
    """The M9 gate (energy precision on the gated cohort) and the default-method recommendation."""
    gate = settings["gate"]
    ...
    if not out["complete"]:
        out["decision"] = f"Incomplete run (methods present: {sorted(out['methods'])}); no decision."
    elif out["m9_default"]:
...
def calibration_reliability(m9_site_days: pd.DataFrame) -> pd.DataFrame:
    ...
    for cohort, g in t.groupby("cohort", sort=True):
        terms = calibration_reliability_terms(g["prob_day"].to_numpy(float), g["rpf"].to_numpy(float))
        rows.append(dict(cohort=cohort, n_days=len(g), **terms))
    return pd.DataFrame(rows)
