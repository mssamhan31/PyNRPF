...
Inputs:  the M9 rows of the site-day table (held-out probability, candidate energies,
         labels), the folds (calibration stations per fold), the settings block
         ``operating_points``.
Outputs: the frontier over c_correct, the (c_correct, c_keep) grid, the target table
         ...
         at chosen points, station-bootstrap intervals, and six figures per cohort.
Key steps: the policy has two controls, AUTO_CORRECT at p >= c_correct and AUTO_KEEP at
         p <= c_keep; the release policy is c_keep = 1 - c_correct. For a precision
         target the threshold is chosen per fold on the calibration stations only, by
         rule L (expected precision from the calibrated probabilities, label-free) or
         rule E (observed precision on the calibration stations' labels), and applied
         once to the held-out station; held-out results are pooled. Nothing here alters
         the headline result at the release c = 0.7.
...
RULES = ("L", "E")
RULE_LABELS = {"L": "rule L: expected precision from p (label-free)",
               "E": "rule E: observed precision on calibration stations"}
RELEASE_RULE = "release"   # the per-station point at the release control c, no target


# ----------------------------------------------------------------------------- one operating point

def prepare(m9_days: pd.DataFrame) -> pd.DataFrame:
...
def thresholds_grid(settings: Settings) -> np.ndarray:
    """The c_correct grid of the configuration."""
...
def frontier(t: pd.DataFrame, settings: Settings) -> pd.DataFrame:
    ...
    return pd.DataFrame([evaluate_point(t, float(c), float(1 - c)) for c in thresholds_grid(settings)])
...
def target_table(t: pd.DataFrame, folds: list[Fold], settings: Settings,
    ...
    """
    op = settings["operating_points"]
    thresholds = thresholds_grid(settings)
        ...
        for applies in ("energy", "day", "both"):
            ...
            for rule in RULES:
                ...
                summaries.append(dict(cohort=cohort, target=float(target), applies=applies, rule=rule,
                                      c_correct_mean=float(sel["c_correct"].mean()),
                                      c_correct_min=float(sel["c_correct"].min()),
                                      c_correct_max=float(sel["c_correct"].max()),
                                      binding=sel["binding"].mode().iloc[0],
...
def bootstrap_targets(t: pd.DataFrame, selections: pd.DataFrame, settings: Settings) -> pd.DataFrame:
    ...
    for (target, applies, rule), sel in selections.groupby(["target", "applies", "rule"], sort=True):
        ...
        for _ in range(draws):
            pick = pd.concat([by_station[s] for s in rng.choice(stations, size=len(stations), replace=True)],
                             ignore_index=True)
        ...
        for k, vals in samples.items():
            ...
            rows.append(dict(cohort=t["cohort"].iloc[0], target=target, applies=applies, rule=rule, metric=k,
                             point=point[k], ci_low=float(np.nanpercentile(arr, lo)),
                             ci_high=float(np.nanpercentile(arr, hi))))
    return pd.DataFrame(rows)


def per_station_points(t: pd.DataFrame, selections: pd.DataFrame, settings: Settings) -> pd.DataFrame:
    """Per-station precision and recall at the configured operating points.

    A point whose rule is ``release`` applies the release control ``m9.c`` symmetrically;
    any other point applies the per-fold thresholds selected for its rule and target.
    """
    op = settings["operating_points"]
    c_release = float(settings["m9"]["c"])
    rows = []
    for point in op["per_station_points"]:
        if point["rule"] == RELEASE_RULE:
            applied = t.assign(c_correct=c_release)
            p = applied["prob_day"].to_numpy(float)
            applied["correct"] = (p >= c_release) & applied["has_window"].to_numpy()
            applied["keep"] = ~applied["correct"] & (p <= 1 - c_release)
        else:
            sel = selections[(selections["rule"] == point["rule"]) & (selections["applies"] == "both")
                             & (selections["target"] == float(point["target"]))]
            applied = apply_selected(t, sel)
        for station, g in applied.groupby("station", sort=True):
            rows.append(dict(point=point["label"], cohort=t["cohort"].iloc[0], station=station,
                             **pooled_from_applied(g)))
    return pd.DataFrame(rows)


# ----------------------------------------------------------------------------- figures

def draw_frontier(axis: Any, front: pd.DataFrame, cohort: str, settings: Settings,
                  marks: pd.DataFrame | None = None) -> None:
    """Precision and recall of automatic corrections against c_correct, the targets band shaded."""
    c = front["c_correct"]
    ...
    axis.plot(c, front["day_recall"], color=COLORS["grey"], lw=2.2, ls="--",
              label="Site-day recall of automatic corrections")
    targets = settings["operating_points"]["targets"]
    axis.axhspan(min(targets), max(targets), color=COLORS["light_white"], alpha=0.8,
                 label=f"precision targets {min(targets):.2f}–{max(targets):.2f}")
    c_release = float(settings["m9"]["c"])
    axis.axvline(c_release, color=COLORS["light_grey"], lw=1, ls=":")
    axis.text(c_release + 0.004, 0.42, f"release c = {c_release:.2f}", rotation=90, fontsize=8,
              color=COLORS["light_grey"], va="bottom")
    if marks is not None:
        for _, m in marks.iterrows():
            axis.axvline(m["c_correct_mean"], color=COLORS["orange"], lw=0.8, ls=":")
            axis.text(m["c_correct_mean"] + 0.004, 0.42, f"rule L, energy ≥ {m['target']:.3g}", rotation=90,
                      fontsize=7, color=COLORS["orange"], va="bottom")
    axis.set_xlabel("Correction threshold c_correct (AUTO_CORRECT when p ≥ c_correct)")
    axis.set_ylabel(f"Held-out score, {COHORT_LABELS[cohort]} site-days")
    axis.set_ylim(0.4, 1.02)
    axis.legend(loc="lower left", fontsize=9)
    style_axis(axis)


def plot_frontier(front: pd.DataFrame, cohort: str, settings: Settings, marks: pd.DataFrame | None,
                  path: Path) -> Path:
    """Fig A: the frontier, one cohort."""
    apply_journal_style()
    figure, axis = plt.subplots(figsize=(8.6, 4.8))
    draw_frontier(axis, front, cohort, settings, marks)
...
def plot_target_vs_achieved(summary: pd.DataFrame, boot: pd.DataFrame, cohort: str, path: Path) -> Path:
    ...
    apply_journal_style()
    figure, axes = plt.subplots(1, 2, figsize=(10.4, 4.6), sharey=True)
    panels = [("energy", "energy_precision", "Reference energy precision"),
              ("day", "day_precision", "Site-day precision")]
    for axis, (applies, metric, label) in zip(axes, panels, strict=True):
        ...
        for rule, color in (("L", COLORS["orange"]), ("E", COLORS["dark_blue"])):
            ...
            b = boot[(boot["applies"] == applies) & (boot["rule"] == rule) & (boot["metric"] == metric)]
            b = b.set_index("target").loc[s["target"]]
            yerr = [s[metric].to_numpy() - b["ci_low"].to_numpy(), b["ci_high"].to_numpy() - s[metric].to_numpy()]
            axis.errorbar(s["target"], s[metric], yerr=yerr, fmt="o-", color=color, capsize=3, lw=1.8,
                          label=RULE_LABELS[rule])
        axis.set_xlabel("Target minimum precision")
        axis.set_title(label, fontsize=11)
        style_axis(axis)
    axes[0].set_ylabel(f"Achieved on held-out {COHORT_LABELS[cohort]} days")
    axes[0].legend(fontsize=8, loc="upper left")
    figure.suptitle("Does asking for a precision deliver it on unseen stations? "
                    "(thresholds set per fold on other stations; station-bootstrap 95% CI)", fontsize=10.5)
    return save_figure(figure, path)[0]
...
def plot_cost(summary: pd.DataFrame, cohort: str, rule: str, path: Path) -> Path:
    ...
    figure, left = plt.subplots(figsize=(8.6, 4.6))
    right = left.twinx()
    step = float(np.min(np.diff(s["target"]))) if len(s) > 1 else 0.02
    right.bar(s["target"], s["fp_per_station_year"], width=0.45 * step, color=COLORS["red"], alpha=0.55,
              label="False corrections per station-year")
    ...
    left.plot(s["target"], s["energy_precision"], "^-", color=COLORS["dark_blue"], lw=2,
              label="Achieved energy precision")
    for x, y, c in zip(s["target"], s["day_recall"], s["c_correct_mean"], strict=True):
        left.annotate(f"c≈{c:.2f}", (x, y), textcoords="offset points", xytext=(0, -14), ha="center", fontsize=7,
                      color=COLORS["grey"])
    left.set_xlabel("Target minimum precision, applied to energy and site-day precision (rule " + rule + ")")
    left.set_ylabel(f"Held-out score, {COHORT_LABELS[cohort]}")
...
def plot_heatmaps(grid: pd.DataFrame, cohort: str, settings: Settings, path: Path) -> Path:
    ...
    panels = [(axes[0], "review_days_per_station_year", "Review days per station-year", "Blues"),
              (axes[1], "auto_fn_per_station_year", "RPF days kept without review, per station-year", "Oranges")]
    for axis, key, title, cmap in panels:
        z = np.full((len(cc), len(ck)), np.nan)
        for i, c_ in enumerate(cc):
            for j, k in enumerate(ck):
                at_c = grid["c_correct"] == c_
                cell = grid["no_review_band"] if k == "no band" else (grid["c_keep"] == k) & ~grid["no_review_band"]
                row = grid[at_c & cell]
                if len(row):
        ...
        for i in range(len(cc)):
            for j in range(len(ck)):
                text = "–" if not np.isfinite(z[i, j]) else f"{z[i, j]:.0f}"
                axis.text(j, i, text, ha="center", va="center", fontsize=8)
        figure.colorbar(im, ax=axis, shrink=0.85)
    figure.suptitle(f"Review load and missed RPF days across both thresholds ({COHORT_LABELS[cohort]}); "
                    "'no band' = c_keep = c_correct", fontsize=10)
    return save_figure(figure, path)[0]
...
def plot_per_station(stations: pd.DataFrame, cohort: str, settings: Settings, path: Path) -> Path:
    ...
    figure, axes = plt.subplots(1, len(points), figsize=(4.2 * len(points), 4.3), sharey=True)
    ...
    figure.suptitle(f"Per-station consistency at candidate operating points (dotted line: {gate:.2f} gate)",
                    fontsize=11)
    return save_figure(figure, path)[0]


def plot_summary(summary: pd.DataFrame, cohort: str, rule: str, path: Path) -> Path:
    """Fig F: one plain figure of what a precision requirement buys and costs."""
    apply_journal_style()
    s = summary[(summary["applies"] == "both") & (summary["rule"] == rule)].sort_values("target")
    figure, axis = plt.subplots(figsize=(8.6, 4.6))
    axis.plot(s["target"], s["energy_precision"], "^-", color=COLORS["orange"], lw=2,
              label="Achieved energy precision on unseen stations")
    axis.plot(s["target"], s["day_recall"], "o-", color=COLORS["grey"], lw=2,
              label="RPF days corrected automatically (recall)")
    ...
    axis.set_title(f"What a precision requirement buys and costs ({COHORT_LABELS[cohort]})")
    axis.legend(loc="lower left", fontsize=9)
    style_axis(axis)
    return save_figure(figure, path)[0]
...
def run_study(m9_days: pd.DataFrame, folds: list[Fold], settings: Settings, folder: Path) -> dict[str, Any]:
    ...
    names = ("frontier", "heatmap", "targets", "selections", "bootstrap", "stations")
    tables: dict[str, list[pd.DataFrame]] = {k: [] for k in names}
    figures: list[Path] = []
    prepared = prepare(m9_days)
    for cohort in ("beta", "alpha"):
        ...
        stations = per_station_points(t, selections, settings)
        for k, v in (("frontier", front), ("heatmap", grid), ("targets", summary),
                     ("selections", selections.assign(cohort=cohort)), ("bootstrap", boot), ("stations", stations)):
            tables[k].append(v)
        marked = (summary["applies"] == "energy") & (summary["rule"] == rule) & summary["target"].isin([0.95, 0.99])
        marks = summary[marked]
        figures += [
            ...
            plot_summary(summary, cohort, rule, folder / f"figF_precision_requirement_{cohort}.png"),
        ]
    outputs = []
    result: dict[str, Any] = {}
