...
OUTPUT_COLUMNS = EXPECTED_COLUMNS + [CONFIDENCE_COLUMN]
ANNOTATION_COLUMNS = [
    "substation_id",
    "date",
    "review_action",
...
DEFAULT_CONFIDENCE = CONFIDENCE_SURE
...
def article_root() -> Path:
    for candidate in workflow_root().parents:
        if candidate.name == "2_journal_article" and (candidate / "dataset").exists():
            return candidate
    raise RuntimeError("Could not locate publication/2_journal_article from oracle workflow.")
...
def default_annotation_path() -> Path:
    return workflow_root() / "manual_oracle_annotations_final_review.csv"


def default_output_dir() -> Path:
    return article_root() / "dataset" / "oracle_data_creation"
...
def apply_annotations(scoped_df: pd.DataFrame, annotations: pd.DataFrame) -> pd.DataFrame:
    ...
    confidence = anns[["substation_id", "date", CONFIDENCE_COLUMN]]
    out = out.merge(confidence, on=["substation_id", "date"], how="left", validate="many_to_one")
    ...
    unknown_confidence = sorted(set(out[CONFIDENCE_COLUMN]) - CONFIDENCE_VALUES)
    if unknown_confidence:
        raise ValueError(f"Unknown output confidence values: {unknown_confidence}.")
...
def dataset_summary(df: pd.DataFrame, dataset_name: str) -> dict[str, object]:
    ...
    if CONFIDENCE_COLUMN in df.columns:
        ...
        summary["sure_confidence_site_days"] = int(
            confidence_site_days[CONFIDENCE_COLUMN].eq(CONFIDENCE_SURE).sum()
        )
        summary["unsure_confidence_site_days"] = int(
            confidence_site_days[CONFIDENCE_COLUMN].eq(CONFIDENCE_UNSURE).sum()
        )
    return summary
