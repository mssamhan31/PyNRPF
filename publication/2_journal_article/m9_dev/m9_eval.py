...
Usage (from the repository root):
    ...
    python publication/2_journal_article/m9_dev/m9_eval.py --run edge --anchors edge --runs-dir <sandbox>/runs

--runs-dir places the run folder outside m9_dev/runs, for sandbox experiments.
"""
...
def score_cohort(days: list[dict], floor: float, variant: str, p_exp: float, sigma_mode: str, stat: str = "gain", missing: str = "abstain_day", edges: str = "any", anchors: str = "nearest") -> pd.DataFrame:
    ...
    for d in days:
        ...
        r = ms.score_siteday(d["y"], d["s"], floor, variant=variant, p_exp=p_exp, sigma=sig, scale=day_scale, stat=stat, missing=missing, edges=edges, anchors=anchors)
...
def run(run_name: str, variant: str, p_exp: float, sigma_mode: str, c: float, stat: str = "gain", missing: str = "abstain_day", edges: str = "any",
        anchors: str = "nearest", runs_dir: pathlib.Path = RUNS) -> None:
    out_dir = runs_dir / run_name
    out_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.time()
    config = dict(run=run_name, variant=variant, p_exp=p_exp, sigma_mode=sigma_mode, c=c, stat=stat, missing=missing, edges=edges, anchors=anchors,
    ...
    for cohort in COHORTS:
        ...
        scores = score_cohort(days, floor, variant, p_exp, sigma_mode, stat, missing, edges, anchors)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--anchors", default="nearest", choices=list(ms.ANCHORS), help="bridge anchor rule; nearest is the frozen default")
    ap.add_argument("--runs-dir", type=pathlib.Path, default=RUNS, help="folder that holds <run>/ (default m9_dev/runs)")
    ap.add_argument("--report-only", action="store_true")
    a = ap.parse_args()
    if a.report_only:
        report(a.runs_dir / a.run)
    else:
        run(a.run, a.variant, a.p, a.sigma, a.c, a.stat, a.missing, a.edges, a.anchors, a.runs_dir)
