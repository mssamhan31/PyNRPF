"""The common site-day population and the interval frames every method receives.

Inputs:  the frozen Alpha and Beta parquet files (interval rows with labels).
...
Key steps: parse timestamps as UTC; count slots per site-day; keep only the days with
         all 96 quarter-hours (the m9_dev population definition, now shared by M7 and
         M8 under section 04 rule 4); give Alpha the confidence 'controlled'.

Column names follow the configuration's ``columns`` block; the evaluation-side names
are fixed: cohort, station, date, slot, y (net load, MW), s (solar, MW), truth.
"""
...
def model_input(intervals: pd.DataFrame, settings: Settings, with_labels: bool = False) -> pd.DataFrame:
    """The four columns the pynrpf package expects (plus labels for training), tz-naive UTC.

    The package strips time zones itself; passing naive UTC timestamps keeps the
    (site, timestamp) key identical on the way in and the way out.
    """
    cols = settings.columns
    out = pd.DataFrame({
