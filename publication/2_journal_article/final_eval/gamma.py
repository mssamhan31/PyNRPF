"""Gamma downstream case study: seven-day-ahead net-load forecasts on raw, M9-corrected and manually corrected data.

Inputs:  the frozen Gamma dataset (Beta station B, one year, every day), the M9 site-day
...
Outputs: the three Gamma series, the data-error metrics, the forecast predictions and
         metrics, the impact table, and the four figures of the archived study in the
         same format: example week, data-error RMSE, grouped forecast RMSE, residuals.
Key steps: apply the held-out M9 decision to each Gamma day (raw kept unless the day is
         ...
         XGBoost once per data condition on targets before September 2024; forecast
         September; score against the manually corrected reference.

Ported from the archived ``_gamma_forecast.py`` and ``_m9_pbm_plotting.py``; the only
change is the correction source (the Phase 3 M9 table replaces the M9-PBM artefact)
and the series label ``M9 corrected``.
"""
...
def apply_m9_correction(gamma: pd.DataFrame, m9_site_days: pd.DataFrame, settings: Settings) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    decisions = m9_site_days[(m9_site_days["station"] == station)][["date", "outcome", "prob_day", "pred_start", "pred_end", "fold_id"]].copy()
...
def _regression_errors(reference: pd.Series, estimate: pd.Series) -> dict[str, float]:
    ...
    return {"n_valid": int(valid.sum()), "rmse_MW": float(np.sqrt(np.mean(residual ** 2))), "mae_MW": float(np.mean(np.abs(residual)))}
...
def data_error_metrics(series: pd.DataFrame, test_start: str, test_end: str) -> pd.DataFrame:
    ...
    for scope, mask in scopes.items():
        ...
        for condition in CONDITIONS[:2]:
            metrics = _regression_errors(series.loc[mask, "manually_corrected_MW"], series.loc[mask, CONDITION_COLUMNS[condition]])
            scope_rows.append({"scope": scope, "data_condition": condition, "data_condition_label": CONDITION_LABELS[condition], **metrics})
        raw = scope_rows[0]
        for row in scope_rows:
            row["rmse_reduction_vs_raw_MW"] = raw["rmse_MW"] - row["rmse_MW"]
            row["mae_reduction_vs_raw_MW"] = raw["mae_MW"] - row["mae_MW"]
            row["rmse_reduction_vs_raw_pct"] = 100 * row["rmse_reduction_vs_raw_MW"] / raw["rmse_MW"] if raw["rmse_MW"] else np.nan
            row["mae_reduction_vs_raw_pct"] = 100 * row["mae_reduction_vs_raw_MW"] / raw["mae_MW"] if raw["mae_MW"] else np.nan
...
def build_forecast_examples(series: pd.DataFrame, series_column: str, target_start: str, target_end: str,
    ...
    ex["target_month_sin"], ex["target_month_cos"] = np.sin(2 * np.pi * (month - 1) / 12), np.cos(2 * np.pi * (month - 1) / 12)
...
def fit_direct_forecasts(train: pd.DataFrame, test: pd.DataFrame, data_condition: str, settings: Settings) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    linear = Pipeline([("imputer", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("regressor", LinearRegression())])
    xg = g["xgboost"]
    xgboost = XGBRegressor(objective=xg["objective"], tree_method=xg["tree_method"], learning_rate=float(xg["eta"]),
                           n_estimators=int(xg["n_estimators"]), max_depth=int(xg["max_depth"]), subsample=float(xg["subsample"]),
                           colsample_bytree=float(xg["colsample_bytree"]), random_state=int(xg["seed"]), n_jobs=4, verbosity=0)
    ...
    audit = pd.DataFrame([{
        ...
        "training_target_start": train.loc[fit_rows, "target_timestamp"].min(), "training_target_end": train.loc[fit_rows, "target_timestamp"].max(),
        "test_examples": len(test), "test_target_start": test["target_timestamp"].min(), "test_target_end": test["target_timestamp"].max(),
...
def forecast_metric_rows(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for (condition, model), group in predictions.groupby(["data_condition", "model"], sort=True):
        rows.append({"data_condition": condition, "data_condition_label": CONDITION_LABELS[condition], "model": model,
                     "model_label": MODEL_LABELS[model], "n_targets_total": len(group), **_regression_errors(group["y_reference"], group["y_pred"])})
...
def forecast_impact_table(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for model, v in wide.iterrows():
        ...
        rows.append({"model": model, "model_label": MODEL_LABELS[model], "raw_rmse_MW": raw, "m9_corrected_rmse_MW": corrected,
                     "manually_corrected_rmse_MW": manual, "m9_rmse_reduction_vs_raw_MW": raw - corrected,
...
def plot_example_week(series: pd.DataFrame, test_start: str, test_end: str, path: Path) -> Path:
    """The test-month week with the largest manual correction: raw, M9 and manual overlaid."""
    apply_journal_style()
    month = series[series["timestamp"].between(_utc(test_start), _utc(test_end, end_of_day=True), inclusive="both")].copy()
    ...
    example = month[month["timestamp"].dt.tz_localize(None).between(week_start, week_start + pd.Timedelta(days=7), inclusive="left")]
    figure, axis = plt.subplots(figsize=(10.2, 4.2))
    axis.plot(example["timestamp"], example["raw_uncorrected_MW"], color=COLORS["grey"], linewidth=1.2, label="Raw uncorrected")
    axis.plot(example["timestamp"], example["m9_corrected_MW"], color=COLORS["orange"], linewidth=1.6, label="M9 corrected")
    axis.plot(example["timestamp"], example["manually_corrected_MW"], color=COLORS["dark_blue"], linewidth=1.2, linestyle="--", label="Manually corrected reference")
...
def plot_forecast_residuals(predictions: pd.DataFrame, path: Path) -> Path:
    ...
    for mi, model in enumerate(MODELS):
        ...
        for ci, condition in enumerate(CONDITIONS):
            residual = data.loc[(data["model"] == model) & (data["data_condition"] == condition), "residual_MW"].dropna()
    ...
    axis.legend(handles=[plt.Rectangle((0, 0), 1, 1, facecolor=c, label=lb) for c, lb in zip(colors, labels, strict=True)], ncol=3, loc="upper center")
    style_axis(axis)
    return _save(figure, path)
...
def run_study(gamma: pd.DataFrame, m9_site_days: pd.DataFrame, settings: Settings, folder: Path) -> dict[str, Any]:
    ...
    for condition, column in CONDITION_COLUMNS.items():
        train = build_forecast_examples(series, column, train_start, train_end, int(g["horizon_days"]), int(g["lookback_days"]))
        test = build_forecast_examples(series, column, test_start, test_end, int(g["horizon_days"]), int(g["lookback_days"]))
    ...
    outputs = {
        ...
        "data_error_metrics": folder / "gamma_data_error_metrics.csv", "forecast_predictions": folder / "gamma_forecast_predictions.parquet",
