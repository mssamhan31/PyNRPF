"""Hand-worked fixtures for the locked energy terms, minimum-demand impact and outcome policy.
...
the band edges; binary outcomes for M7 / M8; the pipeline's identical-key guarantee
on a synthetic cohort run through outcomes and impact.
"""
...
import m9_scorer as ms  # noqa: E402  (registered on sys.path by final_eval)
from final_eval import impact, outcomes  # noqa: E402
from final_eval.common import AUTO_CORRECT, AUTO_KEEP, UNCERTAIN  # noqa: E402
...
def test_energy_terms_partial_overlap_gap_and_noncontiguous():
    ...
    p, r, c = impact.energy_terms(y, truth, proposed)
...
def test_minimum_demand_change_hand_computed():
    ...
    flipped[30:33] = True
    raw, applied, change = impact.minimum_demand(y, flipped)
    assert raw == pytest.approx(0.4)
    assert applied == pytest.approx(-0.6)
    assert change == pytest.approx(-1.0)
    raw2, applied2, change2 = impact.minimum_demand(y, np.zeros(96, bool))
    assert (raw2, applied2, change2) == pytest.approx((0.4, 0.4, 0.0))
    assert np.isnan(impact.minimum_demand(np.full(96, np.nan), flipped)[0])


def test_three_way_policy_at_band_edges():
    c = 0.7
    assert ms.decide(0.7, c) == AUTO_CORRECT
    assert ms.decide(0.6999, c) == UNCERTAIN
    assert ms.decide(0.5, c) == UNCERTAIN
    assert ms.decide(0.3, c) == AUTO_KEEP
    assert ms.decide(0.3001, c) == UNCERTAIN
...
def test_binary_outcomes_and_m9_outcomes_drive_applied_energy():
    ...
    decided = outcomes.attach_outcomes(m7)
    ...
    decided9 = outcomes.attach_outcomes(m9, site)
    ...
    row = impact.siteday_table(decided9, index).iloc[0]
...
def test_methods_share_identical_keys_after_pipeline():
    ...
    for method in ("m7", "m8"):
        for date in ("2024-01-01", "2024-01-02"):
            frames.append(
                outcomes.attach_outcomes(_intervals(method, "beta_A", date, {}, y, truth))
            )
    index = pd.DataFrame(
        {
            "cohort": "beta",
    ...
    tables = [
        impact.siteday_table(pd.concat(frames[i : i + 2], ignore_index=True), index) for i in (0, 2)
    ]
