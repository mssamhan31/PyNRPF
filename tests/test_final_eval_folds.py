...
from final_eval import m8  # noqa: E402
...
def test_default_scopes_are_phase3_and_unknown_scope_is_refused(indexes):
    phase3 = ("all_other_stations_both_cohorts", "other_stations_same_cohort")
    assert fl.fold_scopes(SETTINGS) == phase3
...
def test_phase3_scope_gives_every_fold_its_own_signature(full_indexes):
...
def test_shared_bundle_is_reused_for_an_equal_training_signature(tmp_path, full_indexes):
    ...
    bundle = settings.out("bundles") / "alpha_alpha_A" / "bundle.pkl"
    ...
    assert m8.bundle_uri(settings, alpha_b) == str(bundle)
