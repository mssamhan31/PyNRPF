...
def build(cohort: str, station: str, fold_id: str, phi: float, dates: list[str] | None = None) -> pd.DataFrame:
    ...
    scores = pd.read_parquet(RESULTS / "05_m9" / f"scores_{cohort}.parquet")
    ...
    days = pd.read_parquet(RESULTS / "06_site_days" / "site_days.parquet")
    days = days[(days["method"] == "m9") & (days["station"] == station)]
    days = days[["date", "prob_day", "outcome", "candidate_mwh"]]
    fits = pd.read_csv(RESULTS / "05_m9" / "calibration_fits.csv", float_precision="round_trip").set_index("fold_id")
...
def main() -> None:
    ...
    alpha_scores = pd.read_parquet(RESULTS / "05_m9" / "scores_alpha.parquet")
