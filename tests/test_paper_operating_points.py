...
symmetric two-control policy at c = 0.7 equals the release one-control policy.
"""
...
from paper import operating_points as op  # noqa: E402
from paper.config import Settings  # noqa: E402
from paper.folds import Fold  # noqa: E402
...
SETTINGS = Settings(
    raw={
        ...
        "operating_points": {
            ...
            "recommended_rule": "L",
            "per_station_points": [{"label": "release", "rule": "release", "target": None}],
        },
    }
)
...
def test_rule_l_picks_smallest_threshold_meeting_expected_precision():
    cal = op.prepare(_days("beta_A", [0.99, 0.9, 0.8, 0.6], [1, 1, 1, 0]))
...
def test_selection_never_reads_held_out_labels():
    ...
    thresholds = np.array([0.5, 0.6, 0.7, 0.8, 0.9, 0.99])
    for rule in ("L", "E"):
        a = op.select_fold_threshold(op.prepare(base), fold, 0.9, 0.9, rule, thresholds)
        b = op.select_fold_threshold(op.prepare(flipped), fold, 0.9, 0.9, rule, thresholds)
        assert a["c_correct"] == b["c_correct"]
    leaky = Fold("beta_beta_C", "beta", "beta_C", m9_calibration=("beta_A", "beta_C"))
    with pytest.raises(ValueError, match="held-out"):
        op.select_fold_threshold(op.prepare(base), leaky, 0.9, 0.9, "L", thresholds)


def test_symmetric_policy_at_release_c_matches_one_control_policy():
    t = op.prepare(
    ...
    # The per-station table at the release point applies the same policy.
    stations = op.per_station_points(t, pd.DataFrame(), SETTINGS)
    assert len(stations) == 1 and stations.iloc[0]["point"] == "release"
    assert stations.iloc[0]["corrected_days"] == 2 and stations.iloc[0]["review_days"] == 3
